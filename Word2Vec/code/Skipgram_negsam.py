...
#########################################################################################
def negSamplingCostAndGradient(v, u_index, outputVectors, dataset, K=10):
    """
    negative Sampling cost function for word2vec models
    Implement the cost and gradients for one predicted word vector
    and one target word vector as a building block for word2vec
    models, assuming the negative Sampling cost function.
    ...
    ### YOUR CODE HERE
    total1 = 0
    total2 = np.zeros(outputVectors[u_index].shape)
    gradu_temp = np.zeros(outputVectors.shape)
    index_set = getNegativeSamples(u_index, dataset, K)
    z = sigmoid(np.dot(outputVectors, v))
    for index in index_set:
        total1 += np.log(1 - z[index])
        total2 += outputVectors[index] * z[index]
        gradu_temp[index] += v * z[index]
    ### Cost
    cost_temp = 0
    cost_temp += -np.log(z[u_index]) - total1
    ### Gradient :
    ### Calculate the grad for predictions(1 x dimVectors):
    gradv_temp = outputVectors[u_index] * (z[u_index] - 1) + total2
    ### Calculate the grad for outputVectors (N/2 x dimVectors):
    gradu_temp[u_index] += v * (z[u_index] - 1)
    ### END YOUR CODE
...
def sgd(f, x0, step, iterations, postprocessing=None, useSaved=False,PRINT_EVERY=10):
    ...
    f -- the function to optimize, it should take a single
	 ...
	 "word2vec_sgd_wapper(skipgram, tokens, vec, dataset, C, negSamplingCostAndGradient)"         
    ...
    ### We will train this model(skipgram+negSamplingCostAndGradient) for 40000 times
...
def run():
    ...
    wordVectors = sgd(lambda vec: word2vec_sgd_wrapper(skipgram, tokens, vec, dataset, C, negSamplingCostAndGradient), wordVectors, 0.3, 40000, None, True, PRINT_EVERY=10)
    ...
    visualizeWords = [
    "good", "great", "cool", "brilliant", "well", "amazing","worth",  "enjoyable",
     "sad","dead","death","fear","bomb","gun","happy","life","joy","mad",    "appreciate","clever","crazy","love","delightful","sure"]
